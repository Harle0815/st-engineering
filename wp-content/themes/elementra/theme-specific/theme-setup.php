 ...
 *
 * @package ELEMENTRA
 * @since ELEMENTRA 1.0.22
 */

// If this theme is a free version of premium theme
if ( ! defined( 'ELEMENTRA_THEME_FREE' ) ) {
	define( 'ELEMENTRA_THEME_FREE', false );
}
if ( ! defined( 'ELEMENTRA_THEME_FREE_WP' ) ) {
	define( 'ELEMENTRA_THEME_FREE_WP', false );
}

// If this theme support external updates
if ( ! defined( 'ELEMENTRA_THEME_ALLOW_UPDATE' ) ) {
	define( 'ELEMENTRA_THEME_ALLOW_UPDATE', true );
}

// If this theme uses skins
if ( ! defined( 'ELEMENTRA_ALLOW_SKINS' ) ) {
	define( 'ELEMENTRA_ALLOW_SKINS', true );
}
if ( ! defined( 'ELEMENTRA_DEFAULT_SKIN' ) ) {
	define( 'ELEMENTRA_DEFAULT_SKIN', 'default' );
}
if ( ! defined( 'ELEMENTRA_REMEMBER_SKIN' ) ) {
	define( 'ELEMENTRA_REMEMBER_SKIN', false );
}
...
//-------------------------------------------------------------------------
$GLOBALS['ELEMENTRA_STORAGE'] = array(
...
if ( ! function_exists( 'elementra_get_theme_pro_key' ) ) {
	/**
	 ...
	 */
	function elementra_get_theme_pro_key() {
		$theme_pro_key = get_option( sprintf( 'purchase_code_src_%s', get_template() ) );
		if ( empty( $theme_pro_key ) ) {
			$theme_pro_key = elementra_storage_get( 'theme_pro_key' );
		}
		return $theme_pro_key;
	}
}

if ( ! function_exists( 'elementra_trx_updater_get_theme_info' ) ) {
	add_filter( 'trx_updater_filter_get_theme_info', 'elementra_trx_updater_get_theme_info' );
	/**
	 ...
	 */
	function elementra_trx_updater_get_theme_info( $info ) {
		$engine = elementra_storage_get( 'engine' );
...
if ( ! function_exists( 'elementra_original_theme_slug' ) ) {
	add_filter( 'elementra_filter_original_theme_slug', 'elementra_original_theme_slug' );
	/**
	 ...
	 *
	 * @hooked 'elementra_filter_original_theme_slug'
	 *
	 * @param string $theme_slug  Current slug of the theme.
	 *
	 * @return string             Original slug of the theme.
	 */
	function elementra_original_theme_slug( $theme_slug = '' ) {
		return 'elementra';
	}
}
...
if ( ! function_exists( 'elementra_importer_set_options' ) ) {
	add_filter( 'trx_addons_filter_importer_options', 'elementra_importer_set_options', 9 );
	/**
	 ...
	 */
	function elementra_importer_set_options( $options = array() ) {
		if ( is_array( $options ) ) {
			...
			// Prepare demo data
			$options['demo_url'] = esc_url( elementra_get_protocol() . ':' . elementra_storage_get( 'theme_demofiles_url' ) );
			// Required plugins
			$options['required_plugins'] = array_keys( elementra_storage_get( 'required_plugins' ) );
...
if ( ! function_exists( 'elementra_ocdi_set_options' ) ) {
	add_filter( 'trx_addons_filter_ocdi_options', 'elementra_ocdi_set_options', 9 );
	/**
	 ...
	 * @return array          Processed options with a theme-specific values.
	 */
	function elementra_ocdi_set_options( $options = array() ) {
		if ( is_array( $options ) ) {
			// Prepare demo data
			$options['demo_url'] = esc_url( elementra_get_protocol() . ':' . elementra_storage_get( 'theme_demofiles_url' ) );
			// Required plugins
...
if ( ! function_exists( 'elementra_customizer_theme_setup1' ) ) {
	add_action( 'after_setup_theme', 'elementra_customizer_theme_setup1', 1 );
	/**
	 ...
	 */
	function elementra_customizer_theme_setup1() {
		elementra_storage_set(
			'settings', array(
				...
				'allow_fullscreen'          => false,                   // Allow cases 'fullscreen' and 'fullwide' (if a next settings is true) for the body style in the Theme Options
																		...
																		// (can be removed if filter 'elementra_filter_allow_fullscreen' return false)
