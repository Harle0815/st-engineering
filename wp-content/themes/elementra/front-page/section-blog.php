<div class="front_page_section front_page_section_blog<?php
	$elementra_scheme = elementra_get_theme_option( 'front_page_blog_scheme' );
	if ( ! empty( $elementra_scheme ) && ! elementra_is_inherit( $elementra_scheme ) ) {
		echo ' scheme_' . esc_attr( $elementra_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( elementra_get_theme_option( 'front_page_blog_paddings' ) );
	if ( elementra_get_theme_option( 'front_page_blog_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$elementra_css      = '';
		$elementra_bg_image = elementra_get_theme_option( 'front_page_blog_bg_image' );
...
<?php
	// Add anchor
	$elementra_anchor_icon = elementra_get_theme_option( 'front_page_blog_anchor_icon' );
	$elementra_anchor_text = elementra_get_theme_option( 'front_page_blog_anchor_text' );
...
?>
	...
	<?php
	if ( elementra_get_theme_option( 'front_page_blog_fullheight' ) ) {
	...
	?>
			...
			<?php
			$elementra_css      = '';
			$elementra_bg_mask  = elementra_get_theme_option( 'front_page_blog_bg_mask' );
			$elementra_bg_color_type = elementra_get_theme_option( 'front_page_blog_bg_color_type' );
			if ( 'custom' == $elementra_bg_color_type ) {
				$elementra_bg_color = elementra_get_theme_option( 'front_page_blog_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_blog_content_wrap content_wrap">
			<?php
			// Caption
			$elementra_caption = elementra_get_theme_option( 'front_page_blog_caption' );
			if ( ! empty( $elementra_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_blog_caption front_page_block_<?php echo ! empty( $elementra_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $elementra_caption, 'elementra_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$elementra_description = elementra_get_theme_option( 'front_page_blog_description' );
			if ( ! empty( $elementra_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_blog_description front_page_block_<?php echo ! empty( $elementra_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $elementra_description ), 'elementra_kses_content' ); ?></div>
				<?php
			}
			...
			<div class="front_page_section_output front_page_section_blog_output">
				...
				if ( is_active_sidebar( 'front_page_blog_widgets' ) ) {
					...
					if ( ! elementra_exists_trx_addons() ) {
						...
						elementra_customizer_need_widgets_message( 'front_page_blog_caption', 'ThemeREX Addons - Blogger' );
					}
				}
				?>
			</div>
